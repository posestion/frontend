...
class MyFragment1 : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val rootView = inflater.inflate(com.example.posestion.R.layout.fragment_layout_1, container, false)

        // 새로운 이미지뷰를 찾습니다. (XML에서 적절한 ID를 지정해야 합니다.)
        val newImageView = rootView.findViewById<View>(com.example.posestion.R.id.imageView)
    ...
    private fun showLargeImageDialog() {
        ...
        val dialogView = inflater.inflate(com.example.posestion.R.layout.dialog_large_image, null)

        // 커스텀 레이아웃의 이미지뷰를 찾습니다.
        val largeImageView = dialogView.findViewById<ImageView>(com.example.posestion.R.id.largeImageView)
        ...
        val imageButton1 = dialogView.findViewById<ImageButton>(com.example.posestion.R.id.imageButton)
    ...
    private fun showPopupMenu(view: View) {
        ...
        popupMenu.inflate(com.example.posestion.R.menu.menu_second)
        popupMenu.setOnMenuItemClickListener { item ->
            // 메뉴 아이템을 선택한 경우의 동작을 처리합니다.
            when (item.itemId) {
                com.example.posestion.R.id.item1 -> {
                    // "포즈삭제" 메뉴 아이템을 선택한 경우의 동작을 처리합니다.
                    // 여기서 원하는 동작을 구현하면 됩니다.
                    true
                }
                com.example.posestion.R.id.item2 -> {
